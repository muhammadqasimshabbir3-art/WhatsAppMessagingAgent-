...
export function buildAgentInput(request: RunRequest) {
  const filter = request.contactFilter.trim();
  let prompt = "Scan WhatsApp inbox and reply to unread conversations";
  if (filter) prompt += ` matching "${filter}"`;
  if (request.emailReports) {
    prompt += " and email the HTML report";
  } else {
    prompt += " and generate HTML report";
  }
  ...
  return {
    ...
    max_chats_to_process: request.maxChatsToProcess,
    max_messages_per_chat: request.maxMessagesPerChat,
    max_replies_per_run: request.maxReplies,
    reply_personality: request.replyPersonality,
    enable_message_replies: request.enableMessageReplies,
    reply_only_unread_chats: request.replyOnlyUnreadChats,
