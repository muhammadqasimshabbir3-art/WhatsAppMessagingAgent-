...
function workflowProgressedPastLogin(state: AgentState): boolean {
  return Boolean(
    state.chat_messages?.length ||
      Array.isArray(state.analyzed_messages) ||
...
export function isNodeComplete(nodeName: string, state: AgentState): boolean {
  ...
  switch (nodeName) {
    ...
    case "scan_read_conversations":
      return Boolean(state.chat_messages?.length || state.conversations?.length);
    case "analyze_messages":
      if (workflowProgressedPastAnalyze(state)) return true;
      if (!Array.isArray(state.analyzed_messages)) return false;
      {
        const scraped = state.chat_messages?.length ?? 0;
        if (scraped === 0) return true;
        return state.analyzed_messages.length >= scraped;
      }
    ...
    case "execute_workflow":
      return Boolean(state.html_path || state.chat_messages || state.task_plan_summary);
    default:
      return false;
  }
}
...
export function stateDetailForNode(nodeName: string, state: AgentState): string {
  ...
  switch (nodeName) {
    case "prepare_agent":
      return "Contact config loaded";
    case "decide_agent":
      return state.task_plan_summary ? String(state.task_plan_summary) : "Route selected";
    case "login_whatsapp":
      if (state.logged_in === true || state.whatsapp_logged_in === true) {
        return (
          state.login_detail ??
          state.whatsapp_login_detail ??
      ...
      return (
        state.login_detail ??
        state.whatsapp_login_detail ??
        "⏳ Scan QR in browser (first time only)"
      );
    case "fetch_chat_messages":
    case "scan_read_conversations": {
      const count = state.chat_messages?.length ?? 0;
      const unread = state.unread_chats_found ?? state.read_chats_found ?? 0;
      return `📥 ${unread} unread chat(s), ${count} message(s) extracted`;
    }
    case "analyze_messages": {
      const count = state.analyzed_messages?.length ?? 0;
      const positive = state.positive_messages?.length ?? 0;
    ...
    case "send_replies": {
      ...
      if (posted === 0 && failed === 0 && isPostingDisabled(s)) {
        return "💬 Sending disabled — replies saved in report";
      }
...
export function inferSkippedFromRouting(
  ...
  if (genDone && s.replies_posted == null) {
    if (isPostingDisabled(s)) {
      maybeSkip("send_replies", "Sending disabled in settings");
    } else if ((state.generated_replies?.length ?? 0) === 0) {
      maybeSkip("send_replies", "No replies generated to send");
    } else if (pastReplies) {
      maybeSkip("send_replies", "Skipped by workflow routing");
    }
  }
...
function activeStepHint(stepId: string): string {
  switch (stepId) {
    ...
    case "fetch":
      return "📥 Scanning inbox for unread chats…";
    case "analyze":
      return "🔍 Classifying message sentiment…";
    case "select":
      return "🎯 Selecting messages to reply to…";
    case "generate_replies":
      return "✍️ Generating AI replies (LLM calls — may take a few minutes)…";
    case "send_replies":
      return "💬 Sending replies on WhatsApp Web…";
    case "html_report":
